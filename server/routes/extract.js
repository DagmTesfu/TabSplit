...
router.post('/extract-receipt', (req, res) => {
  ...
  upload(req, res, async (error) => {
    if (error) {
      ...
      return res.status(tooLarge ? 413 : 400).json({
        error: tooLarge ? 'Image is too large (max 5 MB).' :
          error instanceof AiError ? error.message : 'Invalid upload. Send exactly one image file in the image field.',
    ...
    try {
      res.json(await extractReceipt(req.file.buffer, req.file.mimetype));
    } catch (error) {
      if (error instanceof AiError) {
        const status = { NOT_CONFIGURED: 503, PROVIDER_ERROR: 502, PROVIDER_TIMEOUT: 504, INVALID_RESPONSE: 422, INVALID_IMAGE: 400 }[error.code] || 502;
